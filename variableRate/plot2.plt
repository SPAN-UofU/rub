...
filename = "3"
set output "variableRateBuffLCompared.eps"
...
plot "test".filename."metronome.txt" using ($14-start):($2*0.5) title 'True BPM' with steps ls 1 lc 1 lw 1 \
   , "output".filename."buffL300.txt" using ($1-start):2 title 'BuffL = 300' with lines ls 1 lc 3 lw 1 \
   , "output".filename."buffL450.txt" using ($1-start):2 title 'BuffL = 450' with lines ls 1 lc 4 lw 1 \
   , "output".filename.".txt" using ($1-start):2 title 'BuffL = 600' with lines ls 1 lc 2 lw 1 \